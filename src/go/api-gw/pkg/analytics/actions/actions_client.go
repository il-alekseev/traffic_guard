...
// New creates a new actions API client with basic auth credentials.
...
// New creates a new actions API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) PatchAPIV1DetectionsAct(params *PatchAPIV1DetectionsActParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*PatchAPIV1DetectionsActOK, error) {
	...
	success, ok := result.(*PatchAPIV1DetectionsActOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for PatchAPIV1DetectionsAct: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
